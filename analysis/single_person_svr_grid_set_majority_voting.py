    ...
    for fold_idx in range(K_NUM_FOLDS):
        ...
        for lb in testing_fold:
            for X_test, y_test in testing_fold[lb]:
                ...
                correct_cnt = sum([abs(gnd - pred) < 5.0 for gnd, pred in zip(y_test, prediction)])
                ...
                total_test_sets += 1
                correct_test_sets += (abs(decide_set_label(prediction, method='mean') - y_test[0]) < 5.0)
...
for p_C in [2**e for e in range(-8, 11)]:
    for p_gamma in [2**e for e in range(-8, 11)]:
        clf = svm.SVR(C=p_C, gamma=p_gamma)
        ...
        params_str = 'Clf=SVR.rbf, C=%f, gamma=%f' % (p_C, p_gamma)
